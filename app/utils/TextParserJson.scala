...
trait TextParserJson extends ITextParser {
  ...
  implicit val treeWrites = new Writes[Tree]{
    ...
    def writes(t: Tree):JsValue = {
      def writeChildWithIndex(c: Tree):JsObject =
        writes(c) match {
          case o: JsObject => Json.obj("index" -> t.objectIndexOf(c)) ++ o
        }

      if (t.isLeaf)
        Json.obj("category" -> t.value)
      else
        Json.obj("category" -> t.value,
          "constituents" -> t.children.map(writeChildWithIndex))
    }
  }
  ...
  implicit val coreMapWrites = new Writes[(CoreMap, Int)]{
    def writes(c: (CoreMap, Int)) =
      Json.obj(
        "index" -> c._2,
        "phrase_structure" -> Json.toJson(tree(c._1)),
